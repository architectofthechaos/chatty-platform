...
async def create_user(user_data: UserCreateRequest, db: Session = Depends(get_db)) -> UserResponse:
    """
    Create a new user.
    ...
    try:
        # Create new user
        db_user = User(name=user_data.name, handle=user_data.handle)
        ...
        db.rollback()
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="User with this handle already exists")
...
async def update_user(user_id: str, user_data: UserUpdateRequest, db: Session = Depends(get_db)) -> UserResponse:
    """
    Update a user.
...
async def delete_user(user_id: str, db: Session = Depends(get_db)) -> DeleteResponse:
    """
    Delete a user.
...
async def get_user_chatrooms(user_id: str, db: Session = Depends(get_db)) -> UserChatroomListResponse:
    """
    ...
    participants = (
        db.query(ChatroomParticipant, Chatroom)
        .join(Chatroom, ChatroomParticipant.chatroom_id == Chatroom.id)
    ...
    return UserChatroomListResponse(chatrooms=chatroom_responses, total=len(chatroom_responses))
