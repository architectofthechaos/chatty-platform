...
async def create_chatroom_participant(
    participant_data: ChatroomParticipantCreateRequest, db: Session = Depends(get_db)
) -> ChatroomParticipantResponse:
    """
    ...
    try:
        ...
        db_participant = ChatroomParticipant(user_id=participant_data.user_id, chatroom_id=participant_data.chatroom_id)
        ...
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="User is already a participant in this chatroom"
        )


@router.delete("/{participant_id}", response_model=DeleteResponse, status_code=status.HTTP_200_OK)
async def delete_chatroom_participant(participant_id: str, db: Session = Depends(get_db)) -> DeleteResponse:
    """
    ...
    """
    participant = db.query(ChatroomParticipant).filter(ChatroomParticipant.id == participant_id).first()

    if not participant:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Chatroom participant not found")
...
async def remove_user_from_chatroom(user_id: str, chatroom_id: str, db: Session = Depends(get_db)) -> DeleteResponse:
    """
    ...
    participant = (
        ...
        .filter(ChatroomParticipant.user_id == user_id, ChatroomParticipant.chatroom_id == chatroom_id)
        .first()
    )

    if not participant:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User is not a participant in this chatroom")
