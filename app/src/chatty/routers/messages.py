...
async def create_message(message_data: MessageCreateRequest, db: Session = Depends(get_db)) -> MessageResponse:
    """
    ...
    try:
        ...
        if message_data.is_reply and message_data.parent_message_id:
            parent_message = db.query(Message).filter(Message.id == message_data.parent_message_id).first()
...
async def get_message(message_id: str, db: Session = Depends(get_db)) -> MessageResponse:
    """
    Get a message by ID.
...
async def list_messages_by_chatroom(chatroom_id: str, db: Session = Depends(get_db)) -> MessageListResponse:
    """
    ...
    return MessageListResponse(messages=message_responses, total=len(message_responses))


@router.delete("/{message_id}", response_model=DeleteResponse, status_code=status.HTTP_200_OK)
async def delete_message(message_id: str, db: Session = Depends(get_db)) -> DeleteResponse:
    """
    Delete a message.
