...
async def create_chatroom(chatroom_data: ChatroomCreateRequest, db: Session = Depends(get_db)) -> ChatroomResponse:
    """
    ...
    try:
        # Create new chatroom
        db_chatroom = Chatroom(name=chatroom_data.name)
        ...
        db.rollback()
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Chatroom with this name already exists")
...
async def get_chatroom(chatroom_id: str, db: Session = Depends(get_db)) -> ChatroomResponse:
    """
...
async def list_chatrooms(db: Session = Depends(get_db)) -> ChatroomListResponse:
    """
    List all chatrooms.
    ...
    return ChatroomListResponse(chatrooms=chatroom_responses, total=len(chatroom_responses))


@router.put("/{chatroom_id}", response_model=ChatroomResponse)
async def update_chatroom(
    chatroom_id: str, chatroom_data: ChatroomUpdateRequest, db: Session = Depends(get_db)
) -> ChatroomResponse:
    """
    Update a chatroom.
...
async def delete_chatroom(chatroom_id: str, db: Session = Depends(get_db)) -> DeleteResponse:
    """
    Delete a chatroom.
...
async def get_chatroom_users(chatroom_id: str, db: Session = Depends(get_db)) -> ChatroomUserListResponse:
    """
    ...
    participants = (
        ...
        .join(User, ChatroomParticipant.user_id == User.id)
        .filter(ChatroomParticipant.chatroom_id == chatroom_id)
        .all()
    )
    ...
    return ChatroomUserListResponse(users=user_responses, total=len(user_responses))
